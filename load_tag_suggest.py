...
def bulk_index(es_base: str, index: str, docs: List[Dict[str, Any]], chunk_size: int = 2000):
    ...
    for i in range(0, len(docs), chunk_size):
        ...
        for d in chunk:
            ...
            # id ổn định để upsert (tránh trùng)
            doc_id = key_name.lower()
    ...
    # refresh để query thấy ngay
    requests.post(f"{es_base}/{index}/_refresh", timeout=60).raise_for_status()


def main():
    p = argparse.ArgumentParser()
    p.add_argument("--in", dest="inp", required=True, help="Input JSON file (key_name_stats_norm.json)")
    p.add_argument("--es", default="http://localhost:9200", help="ES base url")
    p.add_argument("--index", default="tag_suggest_v1", help="Index name")
    p.add_argument("--chunk", type=int, default=2000, help="Bulk chunk size")
    args = p.parse_args()
