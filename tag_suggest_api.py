...
app = FastAPI(title="TAG Suggest API", version="1.1.0")
...
class SuggestItem(BaseModel):
    ...
    # trả thêm norm full để frontend dùng debug/hiển thị nếu muốn
...
def sort_within_keyname_runs(items: List[SuggestItem]) -> List[SuggestItem]:
    """
    Giữ nguyên thứ tự 'sát nhất' theo ES (order hiện tại),
    CHỈ sort trong từng cụm LIÊN TIẾP có cùng key_name.
    """
    if not items:
        return items
...
def is_relevant(tokens: List[str], doc_norm_full: str) -> bool:
    # chống lạc cho query ascii sai nặng
...
def filter_results(parsed: List[Dict[str, Any]], q_norm: str, tokens: List[str], size: int) -> List[SuggestItem]:
    ...
    for x in parsed:
        ...
        if is_ascii_query(q_norm):
            if not is_relevant(tokens, x["doc_norm_full"]):
                continue
...
def build_query_autocomplete(q_norm: str, size: int) -> Dict[str, Any]:
    """
    Tier 1: ưu tiên sát key_name (full field) trước, rồi mới fallback edge_ngram.
    Không dùng key_time để tránh lệch ranking.
    """
    return {
        "size": size,
        ...
        "query": {
            "bool": {
                "should": [
                    ...
                    # 4) Cuối cùng: edge_ngram để autocomplete mượt
...
def build_query_fuzzy_phrase(q_norm: str, size: int) -> Dict[str, Any]:
    # Tier 2A: fuzzy theo cả cụm trước (ít nhiễu)
    return {
        "size": size,
        "_source": _source_fields(),
        "query": {
...
def build_query_fuzzy_tokens(q_norm: str, size: int) -> Dict[str, Any]:
    # Tier 2B: tách token rồi fuzzy (chỉ khi 2A fail)
...
def build_query_hot(size: int) -> Dict[str, Any]:
    # Tier 3: fallback không rỗng (sort tag_id asc cho ổn định)
    return {
        "size": size,
        "_source": _source_fields(),
        "query": {"match_all": {}},
